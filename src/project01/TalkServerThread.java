...
public class TalkServerThread extends Thread {
	...
	public String chatName = null;//현재 서버에 입장한 클라이언트 스레드 닉네임 저장
	public TalkServerThread(TalkServer ts) {//ts는 TalkServer에서 this와 동일한 주소번지임
		...
		try {
			oos = new ObjectOutputStream(client.getOutputStream());//말하기 객체 생성
			...
			StringTokenizer st = new StringTokenizer(msg,"#");//토큰인 #을 기준으로 문자열 자르기
			st.nextToken();//100 - 100 사용하지 않음 - 그래서 대입연산자로 저장하지 않음
			chatName = st.nextToken();// 누가 입장했는지 필요하니까 저장하기
			ts.jta_log.append(chatName+"님이 입장하였습니다.\n"); //서버측 로그창에 키위님이 입장하였습니다. 로그출력
			//파라미터 자리가 단 두자리뿐 : 앞자리는 제네릭 타입, 뒷자리는 Vector
			//TalkServer 의 run메소드 안에서 인스턴스화됨 ts.globalList.size()=0이가
	...
	//public TalkServerThread(PJ1.talk2.TalkServer talkServer) {
	//}
	//현재 입장해 있는 친구들 모두에게 메시지 전송하기 구현
	public void broadCasting(String msg) {//100#키위
		for(TalkServerThread tst:ts.globalList) {//이전까지는 globalList.size()0이지만 35번 라인에서 하나추가됨
			tst.send(msg); //딱 한 번만 실행이된다. - globalList.size()가 3이면 send메소드가 3번 호출됨
			// tst자리에 this를 쓴다면 내가 보낸 메세지를 나한테 보낸다
			//this.send(msg)가 아니라 왜 tst.send(msg)인가요?
		}
	}
	// 클라이언트에게 말하기 구현할 때 여러명에게 동일한 메소드를 호출해야합니다.
	// 그래서 굳이 메소드로 분리하였다. for문 안에서 send메소드를 호출하면
	// 그 안에서 writeObject가 호출됩니다.
	...
	public void run() {
		...
		try {
			...
			// 라벨문을 while문 앞에 사용했으므로  72~111번 블록을 몽땅 빠져나감 - 112으로 간다.
			run_start://라벨문이다. 강퇴 혹은 강제종료 while문 탈출함
			while(!isStop) {
				// 입장에 대한 처리는 클라이언트가 서버소켓에 접속했을 때
				// TalkServerThread를 인스턴스화 하고 괄호 안에 this를 붙여서 생성자를 호출하였
				// 바로 여기서 입장처리는 끝났다.
				msg = (String)ois.readObject();
				ts.jta_log.append(msg+"\n");//클라이언트가 보낸 메세지를 담는 변수 msg출력하기
				// 클라이언트측에서 보낸 메세지를 서버가 청취하는데 성공했다.
				ts.jta_log.setCaretPosition
				(ts.jta_log.getDocument().getLength());//메세지가 늘어날 떄 자동으로 스크롤 바를 현재 메세지 이동해준다.
				...
				switch(protocol) {
					...
					case 500:{
						String nickName = st.nextToken();
						//현재 서버에 입장해있는 나 자신을 참조하는 스레드 객체
						ts.globalList.remove(this);
						broadCasting(500
								+"#"+nickName);
					}break run_start;//while문 통째로 빠져나옴 -더이상 대화불가
				}/////////////end of switch
			}/////////////////end of while		
			////////여기로 이동한다.
		} catch (Exception e) {
